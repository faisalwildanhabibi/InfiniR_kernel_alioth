...
DECLARE_WORK(susfs_extra_works, susfs_extra_works_handler);
...
bool susfs_is_base_dentry_android_data_dir(struct dentry* base) {
	return (base && !IS_ERR(base) && base->d_inode && base->d_inode->i_mapping && (base->d_inode->i_mapping->flags & BIT_ANDROID_DATA_ROOT_DIR));
}

bool susfs_is_base_dentry_sdcard_dir(struct dentry* base) {
	return (base && !IS_ERR(base) && base->d_inode && base->d_inode->i_mapping && (base->d_inode->i_mapping->flags & BIT_ANDROID_SDCARD_ROOT_DIR));
}
...
static void susfs_update_sus_mount_inode(char *target_pathname) {
	...
	if (mnt->mnt_group_id > 0 && mnt->mnt_group_id < DEFAULT_SUS_MNT_GROUP_ID) {
...
void susfs_update_sus_kstat(void __user **user_info) {
	struct st_susfs_sus_kstat info = {0};
	struct st_susfs_sus_kstat_hlist *new_entry = NULL, *tmp_entry = NULL, *found_entry = NULL;
	struct hlist_node *tmp_node;
	int bkt;
	...
	/* Perform path lookup outside spinlock to prevent sleeping while atomic (ASIL-D / MISRA C) */
	...
	memcpy(&new_entry->info, &found_entry->info, sizeof(found_entry->info));
	new_entry->target_ino = info.target_ino;
...
void susfs_sus_ino_for_generic_fillattr(unsigned long ino, struct kstat *stat) {
	...
	hash_for_each_possible(SUS_KSTAT_HLIST, entry, node, ino) {
		if (entry->target_ino == ino) {
			...
			stat->blksize = entry->info.spoofed_blksize ? entry->info.spoofed_blksize : 4096;
			rcu_read_unlock();
			return;
		}
	}
...
void susfs_auto_add_try_umount_for_bind_mount(struct path *path) {
	...
	/* ISO/IEC 25010 Safety Safeguard: NEVER detach Android FUSE storage, user emulated storage or pass_through views */
	if (!strncmp(dpath, "/storage", 8) ||
	    !strncmp(dpath, "/mnt/user", 9) ||
	    !strncmp(dpath, "/mnt/pass_through", 17) ||
	    !strncmp(dpath, "/mnt/installer", 14) ||
	    !strncmp(dpath, "/mnt/androidable", 16)) {
		goto out_free_pathname;
	}
...
void susfs_set_cmdline_or_bootconfig(void __user **user_info) {
	...
	if (!info) {
		int err = -ENOMEM;
		copy_to_user(&((struct st_susfs_spoof_cmdline_or_bootconfig __user*)*user_info)->err, &err, sizeof(err));
		return;
	}
...
void susfs_get_enabled_features(void __user **user_info) {
	...
	info->err = copy_config_to_buf("CONFIG_KSU_SUSFS_SUS_SU\n", buf_ptr, &copied_size, SUSFS_ENABLED_FEATURES_SIZE);
	if (info->err) goto out_copy_to_user;
	buf_ptr = info->enabled_features + copied_size;
#endif

	if (copied_size < SUSFS_ENABLED_FEATURES_SIZE) {
		info->enabled_features[copied_size] = '\0';
	}
	info->err = 0;
out_copy_to_user:
...
void __attribute__((weak)) ksu_susfs_enable_sus_su(void) {}
void __attribute__((weak)) ksu_susfs_disable_sus_su(void) {}
...
void __weak susfs_start_sdcard_monitor_fn(void) {
}
